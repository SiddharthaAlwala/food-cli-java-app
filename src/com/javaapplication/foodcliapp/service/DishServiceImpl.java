...
public class DishServiceImpl implements DishService{
    ...
    public Dish getDishByName(String name) throws DishNotFoundException {
        Optional<Dish> dishByName = this.dishRepository.findDishByName(name);
        if(dishByName.isEmpty()){
            throw new DishNotFoundException("Dish is not found with the given name: "+ name);
        }
    ...
    public Dish updateDish(Dish dish) throws DishNotFoundException {
        Optional<Dish> byDishId = this.dishRepository.findByDishId(dish.getId());
        if(byDishId.isEmpty()){
            throw new DishNotFoundException("The dish you want to update with the given id is not found.");
